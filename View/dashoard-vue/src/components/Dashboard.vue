...
function selectThread(index: number) {
  selectedThreadIndex.value =
    selectedThreadIndex.value === index ? null : index;
}
    ...
    <div class="container">
      ...
      <div class="process-details" v-if="selectedProcessId !== null">
        ...
        >
          ...
          <p>
            <strong>Memória Working Set:</strong>
            {{ formatMemory(proc.memoryWorkingSet) }}
          </p>
          <p>
            <strong>Memória Committed:</strong>
            {{ formatMemory(proc.memoryCommitted) }}
          </p>
          <p>
            <strong>Memória Privada Committed:</strong>
            {{ formatMemory(proc.privateMemoryCommitted) }}
          </p>
          <p>
            <strong>Memória Reservada:</strong>
            {{ formatMemory(proc.memoryReserved) }}
          </p>
          <p>
            <strong>Memória Heap:</strong>
            {{ formatMemory(proc.memoryHeap) }}
          </p>
          <p>
            <strong>Memória Stack:</strong>
            {{ formatMemory(proc.memoryStack) }}
          </p>
          <p>
            <strong>Memória Code:</strong>
            {{ formatMemory(proc.memoryCode) }}
          </p>
          ...
          <div v-if="selectedThreadIndex !== null">
            ...
            <div v-if="proc.threads[selectedThreadIndex]">
              <p>
                <strong>ID:</strong> {{ proc.threads[selectedThreadIndex].id }}
              </p>
              <p>
                <strong>Priority Base:</strong>
                {{ proc.threads[selectedThreadIndex].priorityBase }}
              </p>
              <p>
                <strong>Priority Delta:</strong>
                {{ proc.threads[selectedThreadIndex].priorityDelta }}
              </p>
            </div>
          </div>
        </div>
